...
export function computeStandings(states) {
  ...
  for (const [g, codes] of Object.entries(D.GROUPS)) {
    const rows = {};
    codes.forEach(c => { rows[c] = { code: c, group: g, p: 0, w: 0, d: 0, l: 0, gf: 0, ga: 0, gd: 0, pts: 0, conduct: 0, str: D.TEAMS[c].str, qualified: false }; });
    for (const fx of D.GROUP_FIXTURES) {
      ...
      if (st.details) {
        st.details.forEach(d => {
          if (d.type === 'yellow' || d.type === 'red') {
            const teamRow = d.isHome ? A : B;
            teamRow.conduct -= (d.type === 'yellow' ? 1 : 4);
          }
        });
      }
    }
...
export function rankThirds(standings) {
  return Object.entries(standings).map(([group, rows]) => ({ ...rows[2], group }))
    .sort((x, y) => y.pts - x.pts || y.gd - x.gd || y.gf - x.gf || y.conduct - x.conduct || y.str - x.str || x.group.localeCompare(y.group))
  ...
  Object.entries(standings).forEach(([group, rows]) => {
    ...
    rows.forEach(row => {
      ...
      const qualifiesInEveryOutcome = groupOutcomes.every(outcome => {
        ...
        const groupsAbleToOvertake = Object.keys(maxThirdPtsByGroup).filter(otherGroup => {
          if (otherGroup === group) return false;
          const maxPts = maxThirdPtsByGroup[otherGroup];
          
          if (maxPts > teamPts) return true;
          if (maxPts < teamPts) return false;
          
          const isOtherComplete = groupComplete(otherGroup, states);
          if (isComplete && isOtherComplete) {
            const myThird = standings[group].find(r => r.code === row.code);
            const otherThird = standings[otherGroup][2];
            const diff = otherThird.pts - myThird.pts || otherThird.gd - myThird.gd || otherThird.gf - myThird.gf || otherThird.conduct - myThird.conduct || otherThird.str - myThird.str || myThird.group.localeCompare(otherThird.group);
            return diff > 0;
          }
...
function resolveBracket(states, standings, allGroupsDone, provider, now) {
  ...
  let thirdsMapping = null;
  if (thirds) {
    thirdsMapping = {};
    const forbidden = { 1: 'E', 2: 'I', 3: 'A', 4: 'L', 5: 'D', 6: 'G', 7: 'B', 8: 'K' };
    const getGroup = (c) => Object.entries(D.GROUPS).find(([g, codes]) => codes.includes(c))[0];
    const assign = (slotIndex, used) => {
      if (slotIndex > 8) return true;
      for (let i = 0; i < 8; i++) {
        if (used[i]) continue;
        const code = thirds[i];
        if (getGroup(code) !== forbidden[slotIndex]) {
          used[i] = true;
          thirdsMapping[slotIndex] = code;
          if (assign(slotIndex + 1, used)) return true;
          used[i] = false;
        }
      }
      return false;
  ...
  const resolveSrc = (s) => {
    ...
    if (s.t) return thirdsMapping ? thirdsMapping[s.t] : null;
    if (s.win || s.lose) {
